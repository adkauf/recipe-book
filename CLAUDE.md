...
- `./scripts/publish.sh [publish|prune]` — copy `output/*/*.pdf` to Google Drive, mirroring the recipes/books/menus structure; `prune` removes stale Drive-only PDFs after confirmation (`publish` never deletes)
